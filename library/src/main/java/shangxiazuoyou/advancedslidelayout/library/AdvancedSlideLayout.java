...
public class AdvancedSlideLayout extends FrameLayout {
    ...
    public AdvancedSlideLayout(Context context) {
        this(context, null);
    }

    public AdvancedSlideLayout(Context context, AttributeSet attrs) {
     ...
     */
    public AdvancedSlideLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        // Style
        final TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AdvancedSlideLayout);

        // Set the side of the screen
        setStickTo(ta.getInt(R.styleable.AdvancedSlideLayout_stickTo, STICK_TO_RIGHT));

        // Sets the shadow drawable
        int shadowRes = ta.getResourceId(R.styleable.AdvancedSlideLayout_shadowDrawable, INVALID_VALUE);
        if (shadowRes != INVALID_VALUE) {
            setShadowDrawable(shadowRes);
        }

        // Sets the shadow size
        mShadowSize = (int) ta.getDimension(R.styleable.AdvancedSlideLayout_shadowSize, 0);

        // Sets the ability to open or close the layer by tapping in any empty space
        changeStateOnTap = ta.getBoolean(R.styleable.AdvancedSlideLayout_changeStateOnTap, true);

        // How much of the view sticks out when closed
        mOffsetDistance = ta.getDimensionPixelOffset(R.styleable.AdvancedSlideLayout_offsetDistance, 0);

        // Sets the size of the preview summary, if any
        mPreviewOffsetDistance = ta.getDimensionPixelOffset(R.styleable.AdvancedSlideLayout_previewOffsetDistance,
                INVALID_VALUE);
